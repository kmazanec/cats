...
2. **Red Team** (`src/cats/agents/red_team/`) — adversarial **LangGraph agent** (R10-followup revised). `agent.py` is the load-bearing graph: an `attacker` LLM node + a `tool_executor` node that dispatches **five tools**: `lookup_regression_history` (only external knowledge channel — confirmed breaches/blocks from past campaigns; Judge verdicts stay invisible to the agent), `propose_attack`, `mutate_attack`, `fire_at_target`, `submit_for_judgment` (terminal; records `self_assessment` for the audit trail, not as a Judge hint). One run per `(category, technique)` scenario; the agent decides how many turns to fire within a USD budget + soft turn cap. The agent's brain uses the `redteam_supervisor` role (DeepSeek, tool-capable, one model across all categories); attack content is authored by per-category `redteam_injection` / `redteam_indirect_injection` / `redteam_exfil` / `redteam_toolabuse` roles (Hermes-class, low-refusal, JSON output only) inside the `propose_attack` tool. Output Filter still gates every payload before egress.
